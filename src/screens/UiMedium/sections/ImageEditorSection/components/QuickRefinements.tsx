...
export const QuickRefinements = ({
  ...
  selectedRefinements = [],
  isLoading = false,
}: QuickRefinementsProps): JSX.Element => {
  return (
            ...
            {quickRefinementsData.map((refinement, index) => {
              ...
              return (
                <Button
                  ...
                  disabled={isLoading}
                  className={`w-full justify-start gap-2 h-auto px-4 py-2.5 disabled:opacity-50 disabled:cursor-not-allowed transition-colors whitespace-nowrap ${isSelected
                    ? "bg-purple-900/50 border-purple-500 text-white hover:bg-purple-900/70"
                    : "bg-[#1a1a24] text-gray-300 border-gray-700 hover:bg-[#1a1a24] hover:text-gray-300"
                    }`}
                >
                  {isLoading ? (
